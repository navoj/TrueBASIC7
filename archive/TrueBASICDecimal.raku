...
class TrueBASICDecimalInterpreter is export {
    ...
    method new(Bool :$debug = False, Bool :$interactive = False) {
        my $compiler = BasicCompiler.new(debug => $debug);
        my $self = self.bless: :$debug, :$interactive, :$compiler;
    ...
    method run-file(Str $filename) {
        ...
        try {
            ...
            if $!compiler.compile() {
                ...
                # (Graphics context check commented out due to module access issues)
                ...
                say "Compilation failed." unless $!debug;
                # Display compilation errors if not already shown in debug mode
                if $!compiler.errors && !$!debug {
                    say "\nCompilation errors:";
                    for $!compiler.errors -> $error {
                        say "  • {$error}";
                    }
                }
                return False;
    ...
    method load-interactive-program(Str $filename, @program-lines) returns Bool {
